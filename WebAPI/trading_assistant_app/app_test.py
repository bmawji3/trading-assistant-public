...
if __name__ == '__main__':
    ...
    print(time.ctime())
    start_date = dt.datetime(2012, 1, 31)
    end_date = dt.datetime(2021, 12, 4)
    ...
    data_directory = os.path.join(cwd, 'data')
    files = [f for f in os.listdir(data_directory) if f.endswith('.csv')]
    SPY_filepath = get_filepath("SPY", base_dir=os.path.join('data'))
    SPY_adj_close = pd.read_csv(get_filepath("SPY", base_dir=os.path.join('data')),
                                index_col="Date",
                                parse_dates=True,
    ...
    for f in os.listdir(data_directory):
        symbol = f.split('.csv')[0]
        temp_data = pd.read_csv('data/' + f,
                                index_col="Date",
                                parse_dates=True,
                                na_values=["nan"])
        ...
        stock_data = ohlcv.loc[start_date:end_date]
        if len(stock_data) == 0:
            continue
        df_indicators = get_technical_indicators_for_symbol(stock_data)
        reddit_fp = os.path.join('reddit_refined', f'{symbol}_rss_wc.csv')
        if not os.path.isfile(reddit_fp):
            continue
        df_reddit = pd.read_csv(reddit_fp)
        df_reddit = df_reddit.set_index('Date')
        df_reddit.index = pd.to_datetime(df_reddit.index)
        ...
        if np.any(np.isnan(X_train)) == False and \
            ...
            y_pred = clf.predict(X_test)
            y_test_ravel = y_test.values.ravel()
            df_y_pred[symbol] = pd.DataFrame(y_pred, index=y_test.index, columns=[f'Y_{symbol}'])
        else:
            df_y_pred[symbol] = pd.DataFrame(np.zeros(len(y_test)), index=y_test.index, columns=[f'Y_{symbol}'])
    print("Modeled")
    print(time.ctime())
    start_date = dt.datetime(2021, 12, 1)
    end_date = dt.datetime(2012, 1, 31)
    while start_date >= end_date:
        buy_signal_recognized_list = []
        sell_signal_recognized_list = []
        for symbol, df in df_y_pred.items():
            try:
                if df_y_pred[symbol].loc[str(start_date.date())][0] == 1.0:
                    buy_signal_recognized_list.append(symbol)
                elif df_y_pred[symbol].loc[str(start_date.date())][0] == -1.0:
                    sell_signal_recognized_list.append(symbol)
            except KeyError as e:
                pass
        buy_signal_recognized_str = '_'.join(buy_signal_recognized_list)
        sell_signal_recognized_str = '_'.join(sell_signal_recognized_list)
        df_buy = pd.DataFrame([[start_date, buy_signal_recognized_str]], columns=['Date', 'Symbols'])
        df_sell = pd.DataFrame([[start_date, sell_signal_recognized_str]], columns=['Date', 'Symbols'])
        if not os.path.isfile('predictions_refined/sell_predictions'):
            df_sell.to_csv('predictions_refined/sell_predictions', index=False)
        else:  # else it exists so append without writing the header
            df_sell.to_csv('predictions_refined/sell_predictions', mode='a', header=False, index=False)
        if not os.path.isfile('predictions_refined/buy_predictions.csv'):
            df_buy.to_csv('predictions_refined/buy_predictions.csv', index=False)
        else:  # else it exists so append without writing the header
            df_buy.to_csv('predictions_refined/buy_predictions.csv', mode='a', header=False, index=False)
        start_date -= delta
    print("Logged")
    print(time.ctime())
